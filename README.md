...
Real-time token detection on Pump.fun (Solana) and PONS (Robinhood ETH):

- **Solana**: Sub-block detection via `connection.onLogs(pumpProgramId)` — captures new mints as they happen
- **Robinhood**: `contract.on('TokenCreated')` via ethers WebSocket
- **Filters**: Keyword blocking, symbol pattern matching, dedup, rate limiting (3 buys/min per chain)
- **Auto-buy**: Per-chain toggle, configurable amounts: 0.3 SOL / 0.02 ETH
...
| `/api/stream` | GET | SSE — all data push |
| `/api/factory/*` | GET/POST | Factory control |
| `/api/sniper/*` | GET/POST | Sniper control |
...
- Real-time SSE data push (no polling)
- 22 pattern detectors with dynamic scoring
- Sniper bot with auto-buy (Solana + Robinhood)
- PONS Token Factory (P&D strategy)
- Multi-chain support (sol, bsc, base, eth, robinhood)
- Unified web dashboard (8 tabs)
